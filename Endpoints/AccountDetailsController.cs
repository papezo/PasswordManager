...
namespace WebApp.Controllers
{
    ...
    public class AccountDetailsController : ControllerBase
    {
       ...
       [HttpPost("forgotpassword")]
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDto forgotPassword)
        {
            ...
            user.TokenExpiry = DateTime.UtcNow.AddHours(29); // Token expiration = 1 Hour
    ...
    public async Task<IActionResult> ResetPassword(int id, [FromBody] ResetPassword model)
    {
        ...
        var account = await _context.AccountDetails.FirstOrDefaultAsync(a => a.Id == id);
        ...
        Console.WriteLine($"TokenExpiry: {account.TokenExpiry}");
        Console.WriteLine($"Current Time: {DateTime.Now}");

        if(account.TokenExpiry < DateTime.Now) 
        {
        ...
        account.ConfirmPassword = model.Password;
        ...
        [HttpGet("email/{email}")]
        public async Task<IActionResult> GetAccountDetailsByEmail(string email)
        {
            var accountDetails = await _context.AccountDetails.FirstOrDefaultAsync(u => u.Email == email);
            if (accountDetails == null)
            {
                return NotFound("Account not found.");
            }
